  ...
  displayList.forEach((post) => {
    ...
    div.innerHTML = `
      ${post.image
        ...
        <a href="profile.html?user=${post.authorId.id}" class="author"><em>By ${post.authorId.name || "Unknown"}</em></a>
...
async function loadSinglePost() {
  ...
  try {
    ...
    container.innerHTML = `
      ...
      <h1>${post.title}</h1>
      <p class="tag">${post.category}</p>
      <p onclick="window.location.href='profile.html?user=${post.authorId.id}'" style="cursor: pointer;"><em>By ${post.authorId.name || "Unknown"}</em></p>
      <small>${new Date(post.date).toLocaleString()}</small>
      <div class="content">
        <p>${post.content}</p>
      </div>
