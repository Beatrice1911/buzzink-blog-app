(function(){const o=document.createElement("link").relList;if(o&&o.supports&&o.supports("modulepreload"))return;for(const t of document.querySelectorAll('link[rel="modulepreload"]'))e(t);new MutationObserver(t=>{for(const a of t)if(a.type==="childList")for(const n of a.addedNodes)n.tagName==="LINK"&&n.rel==="modulepreload"&&e(n)}).observe(document,{childList:!0,subtree:!0});function r(t){const a={};return t.integrity&&(a.integrity=t.integrity),t.referrerPolicy&&(a.referrerPolicy=t.referrerPolicy),t.crossOrigin==="use-credentials"?a.credentials="include":t.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function e(t){if(t.ep)return;t.ep=!0;const a=r(t);fetch(t.href,a)}})();const h="/api/posts",M="/api/auth",_="/api/comments",G="https://i.postimg.cc/KvF0rh0Q/custom-default-avatar.png",X=document.body,V=document.querySelectorAll(".user-icon"),T=document.getElementById("userMenuDetails"),j=document.getElementById("authModal"),ae=document.getElementById("closeModal"),P=document.getElementById("loginTab"),B=document.getElementById("registerTab"),re=document.querySelectorAll(".write-post"),ie=document.querySelector(".search-icon"),z=document.getElementById("mobileSearch"),le=document.querySelector(".menu-toggle"),ce=document.getElementById("mobileMenu"),A=document.querySelector(".logo"),de=document.querySelector(".all-posts-btn"),me=document.getElementById("myPosts"),ue=document.getElementById("savedPosts"),ge=document.getElementById("profile-edit"),fe=document.getElementById("settings"),D=document.getElementById("themeToggle"),O=document.documentElement;function c(s,o="info",r=5e3){const e=document.getElementById("toast-container");if(!e)return;const t=document.createElement("div");t.className=`toast toast-${o}`;const a=document.createElement("i");o==="success"?a.className="fas fa-check-circle":o==="error"?a.className="fas fa-exclamation-circle":a.className="fas fa-info-circle",t.appendChild(a);const n=document.createElement("span");n.textContent=s,t.appendChild(n),e.appendChild(t),setTimeout(()=>{t.style.animation="slideOut 0.5s forwards",t?.addEventListener("animationend",()=>t.remove())},r)}function he(){le?.addEventListener("click",s=>{s.stopPropagation(),T.classList.contains("show")&&T.classList.remove("show"),ce.classList.toggle("active")})}function ye(){A?.addEventListener("click",()=>{window.location.href="index.html"}),de?.addEventListener("click",()=>{window.location.href="all-posts.html"}),me?.addEventListener("click",()=>{window.location.href="my-posts.html"}),ge?.addEventListener("click",()=>{window.location.href="dashboard.html"}),ue?.addEventListener("click",()=>{window.location.href="saved.html"}),fe?.addEventListener("click",()=>{window.location.href="settings.html"})}function pe(){P?.addEventListener("click",()=>{k.classList.remove("hidden"),E.classList.add("hidden"),P.classList.add("active"),B.classList.remove("active")}),B?.addEventListener("click",()=>{E.classList.remove("hidden"),k.classList.add("hidden"),B.classList.add("active"),P.classList.remove("active")}),ae?.addEventListener("click",()=>{j.classList.add("hidden")})}function we(){V.forEach(s=>s?.addEventListener("click",()=>{const o=localStorage.getItem("user"),r=o?JSON.parse(o):null;r&&r.id?(T.classList.toggle("show"),j.classList.add("hidden")):(T.classList.add("hidden"),j.classList.remove("hidden"),P.classList.add("active"),B.classList.remove("active"),k.classList.remove("hidden"),E.classList.add("hidden"),k?.reset(),E?.reset())}))}function ve(){ie?.addEventListener("click",()=>{z.classList.toggle("show"),z.classList.contains("show")&&z.querySelector("input").focus()})}function ke(){re.forEach(s=>{s?.addEventListener("click",o=>{const r=localStorage.getItem("user"),e=r?JSON.parse(r):null;!e||!e.id?(o.preventDefault(),j.classList.remove("hidden"),P.classList.add("active"),B.classList.remove("active"),k.classList.remove("hidden"),E.classList.add("hidden")):(localStorage.removeItem("editSlug"),window.location.href="write.html")})})}function Ee(){D?.addEventListener("change",()=>{D.checked?(O.setAttribute("data-theme","dark"),localStorage.setItem("theme","dark"),A.src="/Images/logo-dark-theme_optimized_.png"):(O.setAttribute("data-theme","light"),localStorage.setItem("theme","light"),A.src="/Images/logo_optimized.png")})}function je(){localStorage.getItem("theme")==="dark"?(O.setAttribute("data-theme","dark"),D&&(D.checked=!0)):O.setAttribute("data-theme","light")}function De(s){s==="dark"?(X.classList.add("dark"),A.src="/Images/logo-dark-theme_optimized_.png"):(X.classList.remove("dark"),A.src="/Images/logo_optimized.png"),localStorage.setItem("theme",s)}function Oe(){he(),pe(),we(),ve(),ke(),ye(),Ee()}const k=document.getElementById("loginForm"),E=document.getElementById("registerForm"),be=document.getElementById("logoutBtn");function R(s){return s?{...s,id:s.id||s._id}:null}window.currentUser=(()=>{const s=localStorage.getItem("user");return s?R(JSON.parse(s)):null})();function L(s){s?.id?V.forEach(o=>o.title=`Logged in as ${s.name}`):(V.forEach(o=>o.title="Click to Login/Register"),T?.classList.remove("show"))}async function H(s){try{const o=await m("/api/users/me");if(!o.ok)return;s=await o.json();const r=document.querySelectorAll(".user-icon");r&&r.forEach(t=>{t.src=s.profilePhoto?.trim()?s.profilePhoto:G});const e=document.querySelectorAll(".avatar");e&&e.forEach(t=>{t.src=s.profilePhoto?.trim()?s.profilePhoto:G}),window.currentUser=s}catch(o){console.warn("Failed to load auth user:",o)}}function Le(){k?.addEventListener("submit",async s=>{s.preventDefault();const o=document.getElementById("loginEmail").value,r=document.getElementById("loginPassword").value;console.log("Login Triggered");const e=await m(`${M}/login`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:o,password:r})}),t=await e.json();console.log("Login response:",t),e.ok||c(`Login failed: ${t.message||"Unknown error"}`,"error");const a=R(t.user);localStorage.setItem("user",JSON.stringify(a)),window.currentUser=a,localStorage.setItem("role",a.role),a.role==="admin"&&(window.location.href="admin.html"),L(a),H(a),authModal.classList.add("hidden"),k.reset(),c(`Welcome back, ${a.name}!`,"success"),Ne()})}function $e(){E?.addEventListener("submit",async s=>{s.preventDefault();const o=document.getElementById("registerName").value,r=document.getElementById("registerEmail").value,e=document.getElementById("registerPassword").value,t=await m(`${M}/register`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({name:o,email:r,password:e})}),a=await t.json();console.log("Register response:",a),t.ok||c(`Registration failed: ${a.message||"Unknown error"}`,"error");const n=R(a.user);localStorage.setItem("user",JSON.stringify(n)),window.currentUser=n,localStorage.setItem("role",n.role),L(n),H(n),authModal.classList.add("hidden"),E.reset(),c(`Welcome, ${n.name}! Your account has been created.`,"success")})}function Se(){be?.addEventListener("click",()=>{Z(),window.location.href="index.html"})}async function He(){try{const s=await m(`${M}/me`);if(!s.ok)throw new Error("Not authenticated");const o=await s.json(),r=R(o);return localStorage.setItem("user",JSON.stringify(r)),window.currentUser=r,L(r),H(r),r}catch{return L(null),H(null),null}}async function Z(s=!1){try{await fetch(`${M}/logout`,{method:"POST",credentials:"include"})}catch(o){console.warn("Logout request failed:",o)}localStorage.removeItem("user"),window.currentUser=null,L(null),s||c("You have been logged out.","info")}function Ie(){const s=document.getElementById("forgotPasswordLink"),o=document.getElementById("forgotPasswordModal"),r=document.getElementById("closeForgotModal"),e=document.getElementById("forgotPasswordForm");s&&s?.addEventListener("click",t=>{t.preventDefault(),o.classList.remove("hidden")}),r&&r?.addEventListener("click",()=>{o.classList.add("hidden")}),e&&e?.addEventListener("submit",async t=>{t.preventDefault();const a=document.getElementById("forgotEmail").value.trim();try{const i=await(await fetch("/api/auth/forgot-password",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:a})})).json();c(i.message||"Check your email for the reset link.","success"),o.classList.add("hidden")}catch(n){console.error(n),c("Failed to send reset link. Try again.","error")}})}function _e(){Le(),$e(),Se(),Ie()}async function m(s,o={}){let r=await fetch(s,{credentials:"include",...o});return r.status===401&&(await Pe()?r=await fetch(s,{credentials:"include",...o}):Z(!0)),r}async function Pe(){try{const s=await fetch(`${M}/refresh`,{method:"POST",credentials:"include"});if(!s.ok)throw new Error("Refresh failed");const o=await s.json();return window.currentUser=o.user,L(o.user),!0}catch{return!1}}async function Re(s){const o=s.dataset.slug;if(!o)return;const r=window.location.pathname.endsWith("post.html"),e=r?document.getElementById("singlePostContainer"):s.closest(".post");if(!e)return;const t=e.querySelector(".comments-section"),a=t?.querySelector(".comments-list");!t||!a||(r||t.classList.toggle("show"),(r||t.classList.contains("show"))&&await ee(o,a))}async function Je(s){if(s.dataset.deleting==="true")return;s.dataset.deleting="true";const o=s.dataset.commentId;if(!confirm("Are you sure you want to delete this comment?")){s.dataset.deleting="false";return}try{const e=await m(`${_}/${o}`,{method:"DELETE"}),t=await e.json();if(e.ok){const a=s.closest(".comment");a&&a.remove();const i=(window.location.pathname.endsWith("post.html")?document.getElementById("singlePostContainer"):s.closest(".post"))?.querySelector(".comment-count");if(i){let l=parseInt(i.textContent)||0;l=Math.max(l-1,0),i.textContent=l,i.title=`${l} comment${l!==1?"s":""}`}c("Comment deleted successfully!","success")}else throw new Error(t.message||"Delete failed")}catch(e){console.error("Error deleting comment:",e),c("Error deleting comment. Please try again.","error")}finally{s.dataset.deleting="false"}}async function ee(s,o,r=3){try{o.innerHTML='<p class="loading-comments">Loading comments...</p>';const e=await m(`${_}/post/${s}?_=${Date.now()}`);if(!e.ok)throw new Error("Failed to fetch comments");const t=await e.json();if(o.innerHTML="",t.length===0){o.innerHTML="<p class='no-comments'>No comments yet. Be the first to comment!</p>";return}const a=t.slice(0,r);if(W(a,o),t.length>r){const n=document.createElement("button");n.classList.add("view-more-btn"),n.textContent=`View all ${t.length} comments`;const i=document.createElement("div");i.classList.add("comments-scroll-container"),i.style.display="none",W(t,i);let l=!1;n?.addEventListener("click",()=>{l=!l,l?(o.innerHTML="",o.appendChild(i),o.appendChild(n),i.style.display="block",n.textContent="View less comments"):(o.innerHTML="",W(a,o),n.textContent=`View all ${t.length} comments`,o.appendChild(n))}),o.appendChild(n)}}catch(e){console.error("Error fetching comments:",e),o.innerHTML="<p class='error-comments'>Failed to load comments.</p>"}}function W(s,o){const r=window.currentUser?.id||window.currentUser?._id;s.forEach(e=>{const t=document.createElement("div");t.classList.add("comment");const a=typeof e.authorId=="object"?e.authorId._id:e.authorId,n=r&&a&&a.toString()===r.toString();t.innerHTML=`
    ...
    `,o.appendChild(t),t.querySelector(".comment-author")?.addEventListener("click",()=>{window.location.href=`profile.html?id=${e.authorId?._id}`})})}async function Be(s,o,r,e){try{const t=await m(`${_}/post/${s}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({text:o})});if(t.ok){const n=(await t.json()).comment,i=document.createElement("div");if(i.classList.add("comment"),i.innerHTML=`
        <div class="comment-header">
      ...
      `,r.prepend(i),e&&e){let l=parseInt(e.textContent)||0;e.textContent=l+1,e.title=`${l+1} comments`}c("Comment posted successfully!","success")}else throw new Error("Failed to post comment")}catch(t){console.error("Error posting comment:",t),c("Failed to post comment","error")}}async function te(s,o){try{const r=await m(`${_}/post/${s}`);if(!r.ok)throw new Error("Failed to fetch comment count");const t=(await r.json()).length;t===0?(o.textContent="0",o.title="No comments yet"):(o.textContent=t,o.title=`${t} comment${t>1?"s":""}`)}catch(r){console.error("Error fetching comment count:",r),o.textContent="0"}}async function Ce(s){const o=s.target.closest(".comment-form"),r=o.querySelector(".comment-form button");if(!o)return;s.preventDefault();const e=o.querySelector(".comment-input"),t=e.value.trim();if(!t)return;let a,n,i,l;window.location.pathname.endsWith("post.html")?(a=document.getElementById("singlePostContainer"),n=a.querySelector(".comments-list"),i=a.querySelector(".comment-btn").dataset.slug,l=a.querySelector(".comment-count")):(a=o.closest(".post"),n=a.querySelector(".comments-list"),i=a.querySelector(".like-btn").dataset.slug,l=a.querySelector(".comment-count"));const d=g=>{r.disabled=g,r.innerHTML=g?'<i class="fa-solid fa-spinner fa-spin"></i>':"Comment"};try{if(d(!0),!window.currentUser){c("Please log in to comment.","error"),e.value="",d(!1);return}await Be(i,t,n,l),e.value=""}catch{d(!1)}finally{d(!1)}}function ze(){document.addEventListener("submit",Ce)}const Te=document.querySelectorAll(".search");let v={all:[],mine:[],saved:[]},$=1,u=Number(sessionStorage.getItem("postsPage"))||1,We=sessionStorage.getItem("postsCategory")||"";sessionStorage.getItem("postsSearch");function Y(s){return s&&s.startsWith("http")?s:"/Images/fallback.jpg"}function Ae(s){s.innerHTML=`<p style="text-align:center; color:gray; font-size: 20px; font-weight: bold;">You haven't made any posts yet...</p>`}async function J(s=1,o=6){try{const r=document.getElementById("categoryFilter")?.value,e=document.querySelectorAll(".search"),t=new URLSearchParams;t.append("page",s),t.append("limit",o),r&&r!=="all"&&t.append("category",r),e.forEach(l=>{if(l){const d=l.value.trim();d&&t.append("search",d)}});const n=await(await m(`${h}?${t.toString()}`)).json();v.all=Array.isArray(n.posts)?n.posts:[],u=n.currentPage??1,$=n.totalPages??1,typeof u<"u"&&sessionStorage.setItem("postsPage",u),sessionStorage.setItem("postsCategory",r&&r!=="all"?r:"");const i=[...e].find(l=>l.value.trim())?.value.trim()||"";sessionStorage.setItem("postsSearch",i),C("allPostsContainer"),Q("allPostsContainer",u,$)}catch(r){console.error("Error fetching posts:",r),c("Something went wrong while displaying posts!","error")}}async function se(s=1,o=6){try{const r=document.querySelectorAll(".search"),e=new URLSearchParams;e.append("page",s),e.append("limit",o),r.forEach(l=>{if(l){const d=l.value.trim();d&&e.append("search",d)}});const t=await m(`${h}/mine?${e.toString()}`);if(!t.ok){const l=await t.text();throw new Error(l||"Failed to fetch your posts")}const a=await t.json();v.mine=Array.isArray(a.posts)?a.posts:[],u=a.currentPage||1,$=a.totalPages||1,sessionStorage.setItem("postsPage",u);const n=[...r].find(l=>l.value.trim())?.value.trim()||"";sessionStorage.setItem("postsSearch",n);const i="myPostsContainer";if(C("myPostsContainer",null,"You haven't made any posts yet..."),Q(i,u,$),v.mine.length===0){const l=document.getElementById("myPostsContainer");l&&Ae(l)}}catch(r){console.error("Error fetching my posts:",r),c("Failed to load your posts!","error")}}function U(s){const o=Math.floor((Date.now()-new Date(s))/1e3),r=[{label:"year",seconds:31536e3},{label:"month",seconds:2592e3},{label:"day",seconds:86400},{label:"hour",seconds:3600},{label:"minute",seconds:60},{label:"second",seconds:1}],e=new Intl.RelativeTimeFormat("en",{numeric:"auto"});for(const t of r){const a=Math.floor(o/t.seconds);if(a>=1)return e.format(-a,t.label)}return"Just now"}function K(s){return s.replace(/\n/g,"<br>")}function C(s,o=null,r=null){const e=window.currentUser?._id||window.currentUser?.id,t=document.getElementById(s);if(!t)return;t.innerHTML="";let a=[];if(s==="allPostsContainer"||s==="featuredPostsContainer"?a=[...v.all]:s==="myPostsContainer"?a=[...v.mine]:s==="savedPostsContainer"&&(a=[...v.saved]),o&&(a=a.slice(0,o)),a.length===0){t.innerHTML=r??'<p style="text-align:center; color:gray; font-size:20px;">No results found...</p>';return}a.forEach(n=>{const i=document.createElement("div");i.classList.add("post");const l=n.content.length>150?n.content.substring(0,150)+"...":n.content,d=typeof n.authorId=="object"&&n.authorId!==null?n.authorId._id:n.authorId,g=typeof n.authorId=="object"&&n.authorId!==null?n.authorId.name:n.authorName||"Unknown",q=e&&String(d)===String(e);i.innerHTML=`
  ...
  `).join("")};function Ue(s){const o=document.getElementById("related-posts-container");if(!s.length){o.innerHTML="<p style='margin: 0 5px;'>No related posts found.</p>";return}o.innerHTML=s.map(r=>`
      <article class="related-post-card">
        <h4><a href="post.html?slug=${r.slug}">${r.title}</a></h4>
        <small>${r.category}</small>
      </article>
    `).join("")}const qe=async s=>{try{const r=await(await m(`${h}/slug/${s}/related`)).json();Ue(r)}catch(o){console.error("Failed to fetch related posts.",o)}},Fe=document.getElementById("savedPostsContainer");async function xe(s=1,o=6){try{const r=document.querySelectorAll(".search"),e=new URLSearchParams;e.append("page",s),e.append("limit",o),r.forEach(i=>{if(i){const l=i.value.trim();l&&e.append("search",l)}});const t=await m(`${h}/saved/me?${e.toString()}`);if(!t.ok)throw new Error("Failed to fetch");const a=await t.json();v.saved=Array.isArray(a.posts)?a.posts:[],u=a.currentPage||1,$=a.totalPages||1,sessionStorage.setItem("postsPage",u);const n=Fe;if(!n)return;if(v.saved.length===0){n.innerHTML="<p>You have no saved posts yet.</p>";return}n.innerHTML=v.saved.map(i=>`
    ...
    `).join(""),document.querySelectorAll(".bookmark").forEach(i=>{i?.addEventListener("click",async l=>{l.stopPropagation();const d=i.dataset.slug;try{await m(`${h}/${d}/unsave`,{method:"POST"}),i.closest(".post-card").remove(),c("Removed from saved posts","success")}catch(g){console.error(g),c("Failed to remove","error")}})}),Q("savedPostsContainer",u,$)}catch(r){console.error(r),container.innerHTML="<p>Error loading saved posts.</p>"}}function Ne(){document.getElementById("allPostsContainer")&&C("allPostsContainer"),document.getElementById("featuredPostsContainer")&&C("featuredPostsContainer",3),document.getElementById("myPostsContainer")&&C("myPostsContainer",null,"You haven't made any posts yet...")}document.getElementById("categoryFilter")?.addEventListener("change",()=>J(1));Te.forEach(s=>s?.addEventListener("keyup",()=>J(1)));function Q(s,o,r){const e=document.getElementById("pagination");if(e){e.innerHTML="";for(let t=1;t<=r;t++){const a=document.createElement("button");a.textContent=t,a.className=t===o?"pg-active":"",a?.addEventListener("click",()=>{s==="myPostsContainer"?se(t):s==="savedPostsContainer"?xe(t):J(t)}),e.appendChild(a)}}}export{m as a,V as b,le as c,Ve as d,Ye as e,We as f,He as g,Je as h,_e as i,Oe as j,ze as k,je as l,ce as m,De as n,Ke as o,J as p,Ge as q,Ne as r,c as s,Re as t,T as u,se as v,Qe as w,xe as x,Z as y};
