(function(){const n=document.createElement("link").relList;if(n&&n.supports&&n.supports("modulepreload"))return;for(const s of document.querySelectorAll('link[rel="modulepreload"]'))t(s);new MutationObserver(s=>{for(const o of s)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&t(r)}).observe(document,{childList:!0,subtree:!0});function a(s){const o={};return s.integrity&&(o.integrity=s.integrity),s.referrerPolicy&&(o.referrerPolicy=s.referrerPolicy),s.crossOrigin==="use-credentials"?o.credentials="include":s.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function t(s){if(s.ep)return;s.ep=!0;const o=a(s);fetch(s.href,o)}})();const y="/api/posts",M="/api/auth",z="/api/comments",le="https://i.postimg.cc/KvF0rh0Q/custom-default-avatar.png",ce=document.body,de=document.querySelectorAll(".user-icon"),L=document.getElementById("userMenuDetails"),fe=document.getElementById("authModal"),$e=document.getElementById("closeModal"),D=document.getElementById("loginTab"),j=document.getElementById("registerTab"),Ie=document.querySelectorAll(".write-post"),Pe=document.querySelector(".search-icon"),Q=document.getElementById("mobileSearch"),he=document.querySelector(".menu-toggle"),O=document.getElementById("mobileMenu"),B=document.querySelector(".logo"),Be=document.querySelector(".all-posts-btn"),Ce=document.getElementById("myPosts"),Te=document.getElementById("savedPosts"),Me=document.getElementById("profile-edit"),Ue=document.getElementById("settings"),H=document.getElementById("themeToggle"),R=document.documentElement;function d(e,n="info",a=5e3){const t=document.getElementById("toast-container");if(!t)return;const s=document.createElement("div");s.className=`toast toast-${n}`;const o=document.createElement("i");n==="success"?o.className="fas fa-check-circle":n==="error"?o.className="fas fa-exclamation-circle":o.className="fas fa-info-circle",s.appendChild(o);const r=document.createElement("span");r.textContent=e,s.appendChild(r),t.appendChild(s),setTimeout(()=>{s.style.animation="slideOut 0.5s forwards",s?.addEventListener("animationend",()=>s.remove())},a)}function Fe(){he?.addEventListener("click",e=>{e.stopPropagation(),L.classList.contains("show")&&L.classList.remove("show"),O.classList.toggle("active")})}function qe(){B?.addEventListener("click",()=>{window.location.href="index.html"}),Be?.addEventListener("click",()=>{window.location.href="all-posts.html"}),Ce?.addEventListener("click",()=>{window.location.href="my-posts.html"}),Me?.addEventListener("click",()=>{window.location.href="dashboard.html"}),Te?.addEventListener("click",()=>{window.location.href="saved.html"}),Ue?.addEventListener("click",()=>{window.location.href="settings.html"})}function Ae(){D?.addEventListener("click",()=>{C.classList.remove("hidden"),T.classList.add("hidden"),D.classList.add("active"),j.classList.remove("active")}),j?.addEventListener("click",()=>{T.classList.remove("hidden"),C.classList.add("hidden"),j.classList.add("active"),D.classList.remove("active")}),$e?.addEventListener("click",()=>{fe.classList.add("hidden")})}function xe(){Pe?.addEventListener("click",()=>{Q.classList.toggle("show"),Q.classList.contains("show")&&Q?.querySelector("input").focus()})}function Ne(){Ie.forEach(e=>{e?.addEventListener("click",n=>{const a=localStorage.getItem("user"),t=a?JSON.parse(a):null;!t||!t.id?(n.preventDefault(),fe.classList.remove("hidden"),D.classList.add("active"),j.classList.remove("active"),C.classList.remove("hidden"),T.classList.add("hidden")):(localStorage.removeItem("editSlug"),window.location.href="write.html")})})}function De(){H?.addEventListener("change",()=>{H.checked?(R.setAttribute("data-theme","dark"),localStorage.setItem("theme","dark"),B.src="/Images/logo-dark-theme_optimized_.png"):(R.setAttribute("data-theme","light"),localStorage.setItem("theme","light"),B.src="/Images/logo_optimized.png")})}function je(){localStorage.getItem("theme")==="dark"?(R.setAttribute("data-theme","dark"),H&&(H.checked=!0)):R.setAttribute("data-theme","light")}function pe(e){e==="dark"?(ce.classList.add("dark"),B.src="/Images/logo-dark-theme_optimized_.png"):(ce.classList.remove("dark"),B.src="/Images/logo_optimized.png"),localStorage.setItem("theme",e)}function ye(){Fe(),Ae(),xe(),Ne(),qe(),De()}let Oe,X;function x(e="allPostsContainer",n=6){clearTimeout(Oe);const a=document.getElementById(e);if(!a)return;let t=a.previousElementSibling?.classList.contains("skeleton-wrapper")?a.previousElementSibling:null;t||(t=document.createElement("div"),t.className="skeleton-wrapper",a.before(t)),t.innerHTML="",t.classList.remove("hidden");for(let s=0;s<n;s++){let o="";e==="savedPostsContainer"?o=`
    ...
    `;const c=l.querySelector(".post-image");c&&(c.onerror=function(){this.onerror=null,this.src="/Images/fallback.jpg"});const h=l?.querySelector(".like-btn"),v=h?.querySelector("i"),m=l?.querySelector(".liked-by");m.dataset.slug=t.slug,m.dataset.likedBy=JSON.stringify(t.likedBy||[]),h.classList.toggle("liked",t.likedByUser),v.className=t.likedByUser?"fa-solid fa-heart":"fa-regular fa-heart",t.likesCount?(m.textContent=t.likesCount===1?`Liked by ${t.likedBy[0]}`:`Liked by ${t.likedBy[0]} and ${t.likedBy.length-1} others`,m.classList.remove("disabled")):(m.textContent="No likes yet",m.classList.add("disabled"));const g=l.querySelector(".comment-count");ke(t.slug,g);const w=document.querySelector(".comments-section"),q=w.querySelector(".comments-list");w&&q&&await ve(t.slug,q,1/0);const k=l.querySelector(".bookmark");k?.addEventListener("click",async()=>{const E=k.dataset.slug,I=k.dataset.saved==="true";if(!window.currentUser){d("Please log in to save posts");return}k.classList.add("clicked"),setTimeout(()=>k.classList.remove("clicked"),200);const Ee=I?`/api/posts/${E}/unsave`:`/api/posts/${E}/save`;try{const A=await u(Ee,{method:"POST"}),Se=await A.json();if(!A.ok)throw new Error(Se.message||"Failed to toggle bookmark");be(!I),d(I?"Removed from saved posts":"Post saved","success")}catch(A){console.error("Failed to toggle bookmark",A),d("Something went wrong","error")}})}catch(a){console.error(a),document.getElementById("singlePostContainer").innerHTML="<p>Error loading post.</p>"}rt(n)}}const nt=async()=>{const n=await(await u(`${y}/trending?limit=5`)).json(),a=document.getElementById("trending-list");a.innerHTML=n.map((t,s)=>`
    <li>
  ...
  `).join("")};function at(e){const n=document.getElementById("related-posts-container");if(!e.length){n.innerHTML="<p style='margin: 0 5px;'>No related posts found.</p>";return}n.innerHTML=e.map(a=>`
    ...
    `).join("")}const rt=async e=>{try{const a=await(await u(`${y}/slug/${e}/related`)).json();at(a)}catch(n){console.error("Failed to fetch related posts.",n)}},it=document.getElementById("savedPostsContainer");async function ie(e,n=6){const a=it;try{const t=b(),s=Number(e??t.page)||1,o=Y()||t.search;$({page:s,search:o},{replace:!0});const r=new URLSearchParams({page:s,limit:n});o&&r.append("search",o),s!==1&&ee();const i=await u(`${y}/saved/me?${r.toString()}`);if(!i.ok)throw new Error("Failed to fetch");const l=await i.json();if(f.saved=Array.isArray(l.posts)?l.posts:[],S=l.totalPages||1,!a)return;if(f.saved.length===0){a.innerHTML="<p>You have no saved posts yet.</p>";return}if(a.innerHTML=f.saved.map(c=>`
    ...
    `).join(""),document.querySelectorAll(".bookmark").forEach(c=>{c?.addEventListener("click",async h=>{h.stopPropagation();const v=c.dataset.slug;try{await u(`${y}/${v}/unsave`,{method:"POST"}),f.saved=f.saved.filter(g=>g.slug!==v),c.closest(".post-card").remove(),d("Removed from saved posts","success");const{page:m}=b();f.saved.length===0&&m>1?ie(m-1):W("savedPostsContainer",m,S)}catch(m){console.error(m),d("Failed to remove","error")}})}),p)return;p=!0,W("savedPostsContainer",s,S)}catch(t){console.error(t),a.innerHTML="<p>Error loading saved posts.</p>"}finally{J(),te(),p=!1}}document.getElementById("categoryFilter")?.addEventListener("change",()=>{$({page:1}),U(1)});let me;se.forEach(e=>e?.addEventListener("keyup",()=>{clearTimeout(me),me=setTimeout(()=>{$({page:1}),Le(),U(1),ne(1),ie(1)},400)}));function W(e,n,a){const t=document.getElementById("pagination");if(t){t.innerHTML="";for(let s=1;s<=a;s++){const o=document.createElement("button");o.textContent=s,o.className=s===n?"pg-active":"",o?.addEventListener("click",async()=>{$({page:s}),U(s)}),t.appendChild(o)}}}async function lt(e){const n=e.dataset.slug,a=e.querySelector("i"),t=e.querySelector(".like-count"),s=e.closest(".post-interactions-container")?.querySelector(".liked-by");if(!window.currentUser){d("Please log in to like or unlike posts.","error");return}const o=e.classList.contains("liked");try{const r=await u(`/api/posts/${n}/${o?"unlike":"like"}`,{method:"POST"}),i=await r.json();if(r.ok){e.classList.toggle("liked",!o),a.className=o?"fa-regular fa-heart":"fa-solid fa-heart";const l=i.likesCount??i.likes??0;t.textContent=l,s&&(l?(s.textContent=l===1?"1 like":`${l} likes`,s.classList.remove("disabled")):(s.textContent="No likes yet",s.classList.add("disabled")),Array.isArray(i.likedBy)&&(s.dataset.slug=n,s.dataset.likedBy=JSON.stringify(i.likedBy)))}else d(`Failed to update likes: ${i.message}`,"error")}catch(r){console.error("Like action failed:",r),d("Error updating like. Please try again.","error")}}const ct=async e=>{const n=e.dataset.slug,a=`${window.location.origin}/post/${n}`,t=`shared_${n}`;try{navigator.share?await navigator.share({title:"BuzzInk",text:"Check out this post on BuzzInk",url:a}):(await navigator.clipboard.writeText(a),d("Link copied to clipboard!","success")),sessionStorage.getItem(t)||(sessionStorage.setItem(t,"true"),u(`/api/posts/${n}/share`,{method:"POST"}).catch(()=>{}));const s=e.querySelector(".share-count");s&&(s.textContent=Number(s.textContent)+1)}catch(s){d("Failed to share post. Please try again.","error"),console.error("Share cancelled or failed",s)}};let Z=!1;async function dt(e){if(Z)return;const n=encodeURIComponent(e),a=document.getElementById(`likesModal-${n}`),t=document.getElementById(`likesList-${n}`);if(!(!a||!t)&&!a.classList.contains("active")){Z=!0,a.classList.remove("hidden"),requestAnimationFrame(()=>a.classList.add("active")),t.innerHTML="<li>Loading...</li>";try{const o=await(await u(`/api/posts/${e}/likes`)).json();if(t.innerHTML="",!Array.isArray(o.users)||o.users.length===0){t.innerHTML="<li>No likes yet</li>";return}o.users.forEach(r=>{const i=document.createElement("li");i.textContent=r,t.appendChild(i)})}catch(s){t.innerHTML="<li>Failed to load likes</li>",console.error("Failed to fetch likes:",s)}}}function ut(e){const n=encodeURIComponent(e),a=document.getElementById(`likesModal-${n}`);a&&(Z=!1,a.classList.remove("active"),setTimeout(()=>{a.classList.add("hidden")},300))}function mt(){document.addEventListener("click",async e=>{const n=e.target.closest(".edit-btn");if(n){e.preventDefault(),e.stopPropagation(),st(n.dataset.slug);return}const a=e.target.closest(".delete-btn");if(a){e.preventDefault(),e.stopPropagation(),tt(a.dataset.slug);return}const t=e.target.closest(".like-btn");if(t){e.preventDefault(),lt(t);return}const s=e.target.closest(".comment-btn");if(s){e.preventDefault(),Ke(s);return}const o=e.target.closest(".share-btn");if(o){e.preventDefault(),ct(o);return}const r=e.target.closest(".likes-info");if(r&&!r.classList.contains("disabled")){e.preventDefault(),e.stopPropagation();const g=r.dataset.slug;if(!g)return;dt(g);return}const i=document.querySelector(".likes-modal.active");if(i&&!i.contains(e.target)){const g=i.id.replace("likesModal-","");ut(g)}const l=e.target.closest(".delete-comment-btn");l&&(e.preventDefault(),e.stopPropagation(),Qe(l));const c=e.target.closest(".menu-btn"),h=e.target.closest(".menu-options");if(!c&&!h&&document.querySelectorAll(".menu-options").forEach(g=>g.classList.add("hidden")),c&&c.nextElementSibling.classList.toggle("hidden"),L?.classList.contains("show")&&!L.contains(e.target)&&!e.target.closest(".user-icon")&&L.classList.remove("show"),O?.classList.contains("active")&&!O.contains(e.target)&&!he.contains(e.target)&&O.classList.remove("active"),!e.target.closest(".user-icon"))return;e.stopPropagation();const m=window.currentUser;m&&m.id?(L.classList.toggle("show"),authModal.classList.add("hidden")):(L.classList.remove("show"),authModal.classList.remove("hidden"),loginTab.classList.add("active"),registerTab.classList.remove("active"),loginForm.classList.remove("hidden"),registerForm.classList.add("hidden"))})}document.getElementById("canonicalUrl")?.setAttribute("href",window.location.href);"scrollRestoration"in history&&(history.scrollRestoration="manual");function gt(){const e=`scroll:${window.location.pathname}${window.location.search}`,n=sessionStorage.getItem(e);n!==null&&(window.scrollTo(0,Number(n)),sessionStorage.removeItem(e))}let ge=!1;async function K(){if(ge)return;ge=!0;const e=window.location.pathname,{page:n}=b();e==="/"||e.endsWith("index.html")?(N(),x("featuredPostsContainer",3),await Le(),await nt()):e.endsWith("my-posts.html")?(N(),x("myPostsContainer",6),await ne(n)):e.endsWith("post.html")?await ue():e.endsWith("saved.html")?(N(),x("savedPostsContainer",6),await ie(n)):e.startsWith("/post/")?ue():(N(),x(),await U(n)),gt()}document.addEventListener("DOMContentLoaded",async()=>{const e=await ze();window.currentUser=e,Ve(),ye(),mt(),Ze(),je();const n=localStorage.getItem("theme")||"light";pe(n),await K(),window.location.pathname.endsWith("write.html")&&!localStorage.getItem("editSlug")&&localStorage.removeItem("editSlug"),ot()});window.addEventListener("pageshow",e=>{if(e.persisted){const n=localStorage.getItem("theme")||"light";pe(n),ye()}});window.addEventListener("popstate",()=>{K()});export{u as a,we as l,d as s};
